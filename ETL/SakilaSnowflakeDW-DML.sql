...
SELECT
    ...
    df.film_key,
    dstore.store_key,
    drent.time_key AS rental_date_key,
    dret.time_key AS return_date_key,
    1 AS count_rentals,
...
JOIN sakila_snowflake.dim_customer dc ON c.customer_id = dc.customer_id
JOIN sakila_snowflake.dim_staff dsf ON s.staff_id = dsf.staff_id
JOIN sakila_snowflake.dim_film df ON f.film_id = df.film_id
JOIN sakila_snowflake.dim_store dstore ON st.store_id = dstore.store_id
JOIN sakila_snowflake.dim_time drent ON DATE(r.rental_date) = drent.date
LEFT JOIN sakila_snowflake.dim_time dret ON DATE(r.return_date) = dret.date;



SELECT COUNT(*) FROM sakila_snowflake.fact_rental;
